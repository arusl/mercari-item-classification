...
def run_training():
   ...
   # split categories into 5 column
    ...
    # use 100% of the downsampled dataset 
    ...
    # train and evaluate on training split
    predicted = pipeline.pipeline.fit(X_train, y_train).predict(X_train)
    print(metrics.classification_report(y_train, predicted, target_names=df['cat1'].unique()))

    # evaluate on test split
    preds = pipeline.pipeline.predict(X_test)
    print(metrics.classification_report(y_test, preds, target_names=df['cat1'].unique()))

    joblib.dump(pipeline.pipeline, config.PIPELINE_NAME)
